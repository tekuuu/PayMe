    ...
    const run = async () => {
      ...
      try {
        ...
        try {
          ...
          if (isSimulationRevert) {
            try {
              console.warn("[useFHEDecrypt] Simulation revert detected, retrying once after backoff...");
              setMessage("Relayer simulation failed; retrying decrypt...");
              ...
              console.error("[useFHEDecrypt] userDecrypt RETRY FAILED:", retryErr);
              console.error("[useFHEDecrypt] Full retry error object:", JSON.stringify(retryErr, Object.getOwnPropertyNames(retryErr as object)));
              const retry = retryErr as unknown as { name?: string; message?: string };
              const retryCode = retry && typeof retry === "object" && "name" in (retry as any) ? (retry as any).name : code;
              const retryMsg = retry && typeof retry === "object" && "message" in (retry as any) ? (retry as any).message : msg;
              setError(`${retryCode}: ${retryMsg}`);
              setMessage("FHEVM userDecrypt failed");
              return;
            }
          } else {
            console.error("[useFHEDecrypt] userDecrypt FAILED:", e);
            console.error("[useFHEDecrypt] Full error object:", JSON.stringify(e, Object.getOwnPropertyNames(e as object)));
            setError(`${code}: ${msg}`);
            setMessage("FHEVM userDecrypt failed");
            return;
          }
